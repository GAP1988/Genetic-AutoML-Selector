...
file_path = 'used_device_data.csv'
...
def evaluate(individual):
    ...
    if model_type == 'SVR':
        params['C'] = individual[1] * 10  
        params['gamma'] = individual[2] * 0.1
    elif model_type == 'RandomForestRegressor':
        params['n_estimators'] = int(individual[1] * 100)
        params['max_depth'] = int(individual[2] * 10) + 1 
    elif model_type == 'KNeighborsRegressor':
        params['n_neighbors'] = int(individual[1] * 20) + 1 
        params['leaf_size'] = int(individual[2] * 50) + 1
    elif model_type == 'DecisionTreeRegressor':
        params['max_depth'] = int(individual[1] * 20) + 1 
        params['min_samples_split'] = int(individual[2] * 10) + 2
...
if model_type == 'SVR':
    params['C'] = best_ind[1] * 10  
    params['gamma'] = best_ind[2] * 0.1
elif model_type == 'RandomForestRegressor':
    params['n_estimators'] = int(best_ind[1] * 100)
    params['max_depth'] = int(best_ind[2] * 10) + 1  
elif model_type == 'KNeighborsRegressor':
    params['n_neighbors'] = int(best_ind[1] * 20) + 1 
    params['leaf_size'] = int(best_ind[2] * 50) + 1 
elif model_type == 'DecisionTreeRegressor':
    params['max_depth'] = int(best_ind[1] * 20) + 1
    params['min_samples_split'] = int(best_ind[2] * 10) + 2 
